...
TheoryPrediction TheoryPredictor::predict(const EVContext& context, const EnsembleContext& ensemble) {
    ...
    if (ensemble.peerCount > 0) {
        ...
        if (peerChordVotes[1] > 0 && (peerChordVotes[0] > 0 || peerChordVotes[3] > 0)) ensembleInfluence[2] += 200;
    }
...
int CorrelationEngine::calculateTransposition(const EVContext& context, int baseNote) {
    ...
    int geoWeight = (awareness.dataQuality > 60) ? 10 : 2;
...
void CorrelationEngine::process(const EVContext& context, int baseNote) {
    ...
    if (ensemble.peerCount > 0) {
        int moodSum = 0;
        int totalPeerClashRate = 0;
        int totalPeerDissonance = 0;
        for (int i = 0; i < 4; i++) {
            if (ensemble.peers[i].active) {
                moodSum += (int)ensemble.peers[i].mood;
                totalPeerClashRate += ensemble.peers[i].clashRate;
                totalPeerDissonance += ensemble.peers[i].dissonance;
            }
        }
        int avgMood = moodSum / ensemble.peerCount;
        awareness.moodAlignment = 100 - abs((int)theory.localMood - avgMood) * 33;

        // Mutual Confidence: penalized if peers are clashing
        if (totalPeerClashRate / ensemble.peerCount > 40) awareness.moodAlignment -= 20;

        // Collective Tension Awareness (Cool-down)
        if (totalPeerDissonance / ensemble.peerCount > 70 && awareness.confidence > 0.6f) {
            theory.localMood = MOOD_RESOLUTION; // Force stabilization
            awareness.moodAlignment += 10; // "Doing my part" boost
        }

        // Flow State Synchronization: confidence boost from peer virtuosity
        bool peerVirtuoso = false;
        for(int i=0; i<4; i++) {
            if(ensemble.peers[i].active && (millis() - ensemble.peers[i].firstSeen > 60000) && ensemble.peers[i].intensity > 110) peerVirtuoso = true;
        }
        if (peerVirtuoso) awareness.moodAlignment = constrain(awareness.moodAlignment + 15, 0, 100);
    ...
    float targetConfidence = (awareness.moodAlignment / 100.0f) * (1.0f - awareness.peerClashRate / 150.0f);
    awareness.confidence = (awareness.confidence * 0.9f) + (constrain(targetConfidence, 0.0f, 1.0f) * 0.1f);

    // Flow State: sustained high confidence
    if (awareness.confidence > 0.95f) awareness.flowState = (awareness.flowState * 9 + 100) / 10;
    else awareness.flowState = (awareness.flowState * 8) / 10;

    // Social Mirroring: if confident COMPING, nudge phase toward LEAD
    if (localRole == ROLE_COMPING && awareness.confidence > 0.8f && ensemble.peerCount > 0) {
        for(int i=0; i<4; i++) {
            if(ensemble.peers[i].active && ensemble.peers[i].role == ROLE_LEAD) {
                if (abs(awareness.currentPhase - ensemble.peers[i].phase) > 10) {
                    awareness.currentPhase = (awareness.currentPhase * 8 + ensemble.peers[i].phase * 2) / 10;
                }
                break;
            }
        }
...
void CorrelationEngine::performMusicalLogic(const EVContext& context, const PsychoacousticPrediction& pp, const TheoryPrediction& tp, int transpositionOffset, const EnsembleContext& ensSnap) {
    ...
    int baseDelay = map(100 - pp.energeticIntensity, 0, 100, 200, 800);

    // Social Pulse: sync toward peer intensity
    if (ensSnap.peerCount > 0) {
        int avgIntensity = 0;
        for(int i=0; i<4; i++) if(ensSnap.peers[i].active) avgIntensity += ensSnap.peers[i].intensity;
        avgIntensity /= ensSnap.peerCount;
        int peerDelay = map(100 - avgIntensity, 0, 100, 200, 800);
        baseDelay = (baseDelay * 7 + peerDelay * 3) / 10;
    }

    int actualDelay = constrain(baseDelay - tempoOffset, 100, 1200);

    // Phase Alignment: nudge local delay to match ensemble rhythmic phase
    if (ensSnap.peerCount > 0) {
        int avgPeerPhase = 0;
        for(int i=0; i<4; i++) if(ensSnap.peers[i].active) avgPeerPhase += ensSnap.peers[i].phase;
        avgPeerPhase /= ensSnap.peerCount;

        // If we are ahead of peers, slow down; if behind, speed up
        int phaseDiff = awareness.currentPhase - avgPeerPhase;
        if (abs(phaseDiff) > 5) {
            actualDelay += phaseDiff * 2; // nudge
        }
    }
    ...
    // Flow State novelty boost
    if (awareness.flowState > 80) novelty = constrain(novelty + 20, 0, 100);

    if (useJazznet) {
        if (localRole == ROLE_COMPING) novelty /= 2;
        for (int offset = 0; offset < jazznetSize; offset += 4) {
            awareness.currentPhase = (offset * 100) / jazznetSize;
...
void sendChord(const int* chordDefinition, int chordDefSize, int transpositionOffset, int velocity) {
  ...
  if (engine.localRole == ROLE_BASS) { minOct = 2; maxOct = 4; }
    ...
    // Flow State: Extreme registers
    if (engine.awareness.flowState > 80) {
        minOct--; maxOct++;
    }
  ...
  for (int i = 0; i < chordDefSize && currentChordNotesCount < MAX_NOTES_PER_CHORD; ++i) {
    ...
    if (bestNote >= 0 && bestNote <= 127) {
      ...
      bool ensembleClash = isDissonant(bestNote, ensembleNotes, ensembleNotesCount);
      ...
      if (localDissonant || ensembleClash) {
        ...
        for (int c : corrections) {
          ...
          if (!isDissonant(correctedNote, currentChordNotes, currentChordNotesCount) &&
              !isDissonant(correctedNote, ensembleNotes, ensembleNotesCount)) {
